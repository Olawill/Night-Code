...
const ChatMessage = ({ msg, streaming }: ChatMessageProps) => {
  if (msg.role === "user") {
  ...
  return (
    <BotMessage
      ...
      streaming={streaming}
      interrupted={msg.metadata?.status === "interrupted"}
    />
  );
};

const SessionChat = ({ session, initialPrompt }: SessionChatProps) => {
  const [initialMessages] = useState(
    () => session.messages as unknown as Message[],
  );
  const { mode, model } = usePromptConfig();
  const { isTopLayer } = useKeyboardLayer();
  const { messages, error, status, submit, abort, interrupt } = useChat(
    session.id,
    initialMessages,
  );
